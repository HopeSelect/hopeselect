    ...
    <AppShell>
      <div className="flex w-full flex-1 flex-col lg:flex-row">
        ...
        <aside className="w-full shrink-0 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-5 lg:w-80 lg:border-l lg:border-t-0">
          <h2 className="mb-4 font-medium text-gray-900 dark:text-gray-100">Nova nutricionista</h2>
          <NutricionistaForm />

          {todas.length > 0 && (
            <div className="mt-8">
              <h2 className="mb-2 font-medium text-gray-900 dark:text-gray-100">Cadastradas</h2>
              <div className="space-y-2">
                {todas.map((n) => (
                  <div key={n.id} className="flex items-center justify-between gap-2 rounded-md border border-gray-200 dark:border-gray-700 p-2 text-sm">
                    <span className={n.ativo ? 'text-gray-900 dark:text-gray-100' : 'text-gray-400 dark:text-gray-500'}>
                      {n.nome}
                      {!n.ativo && ' (inativa)'}
                    </span>
