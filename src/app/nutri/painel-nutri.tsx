...
export function PainelNutri({
  ...
  async function confirmarAlocacao(alunoId: string, tentativa = 1) {
    const { data, error } = await supabase
      .from('atendimentos_nutricionista')
      .select('id, aluno_id, nutricionista_id, inicio, alunos(id, nome, classificacao, alertas, ultimo_acesso, restricoes, foto_url)')
      .eq('aluno_id', alunoId)
      .is('fim', null)
      .maybeSingle()
    ...
    if (error) console.error('Erro ao confirmar alocação (nutri):', error.message)
    if (tentativa < 6) {
