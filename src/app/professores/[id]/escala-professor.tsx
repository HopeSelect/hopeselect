...
export function EscalaProfessor({
  ...
  return (
    <div className="mt-6 rounded-xl border border-gray-200 bg-white p-5">
      <h2 className="mb-1 font-medium text-gray-900">Escala semanal</h2>
      ...
      <div className="space-y-3">
        {porDia.map((dia) => (
          <div key={dia.indice} className="flex items-start gap-3">
            <span className="w-20 shrink-0 pt-1.5 text-sm font-medium text-gray-700">{dia.nome}</span>
            <div className="flex flex-1 flex-wrap gap-1.5">
              {dia.blocos.length === 0 && <span className="pt-1.5 text-xs text-gray-300">—</span>}
              {dia.blocos.map((b) => (
                <span
                  key={b.id}
      ...
      <form action={aoSubmeter} className="mt-4 flex flex-wrap items-end gap-2 border-t border-gray-100 pt-4">
        <label className="text-xs font-medium text-gray-700">
          Dia
          <select name="dia_semana" defaultValue="1" className="mt-1 block rounded-md border border-gray-300 px-2 py-1.5 text-sm">
