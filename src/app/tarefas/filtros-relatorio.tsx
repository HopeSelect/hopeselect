...
export function FiltrosRelatorioTarefas({
  ...
  function montarParams(extra: { professor?: string; tipo?: string; status?: string }) {
    const params = new URLSearchParams()
    const professor = extra.professor ?? professorSelecionado
    const tipo = extra.tipo ?? tipoSelecionado
    const status = extra.status ?? statusSelecionado
  ...
  function aoTrocarProfessor(valor: string) {
    const params = montarParams({ professor: valor })
    if (dias === null) {
      params.set('de', de)
      params.set('ate', ate)
    } else {
  ...
  function aoTrocarTipo(valor: string) {
    const params = montarParams({ tipo: valor })
    if (dias === null) {
      params.set('de', de)
      params.set('ate', ate)
    } else {
  ...
  function aoTrocarStatus(valor: string) {
    const params = montarParams({ status: valor })
    if (dias === null) {
      params.set('de', de)
      params.set('ate', ate)
    } else {
    ...
    <div className="flex flex-wrap items-end gap-3">
      ...
      <label className="text-sm text-gray-700">
        ...
        <select
          ...
          onChange={(e) => aoTrocarStatus(e.target.value)}
          className={`mt-1 block ${campo}`}
        >
          <option value="">Todos</option>
          {STATUS_TAREFA_SELECIONAVEIS.map((s: StatusTarefa) => (
