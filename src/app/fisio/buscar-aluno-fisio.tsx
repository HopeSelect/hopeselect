...
import { alocarAlunoFisio } from './actions'

export function BuscarAlunoFisio({
  fisioterapeutaId,
  fisioterapeutaNome,
  onFechar,
  onAlocado,
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      let query = supabase
        .from('alunos')
        .select('id, nome, classificacao, alertas, ultimo_acesso, restricoes, foto_url')
        .order('nome')
        .limit(200)
  ...
  async function alocar(aluno: AlunoResumo) {
    ...
    setErro(null)
    const resultado = await alocarAlunoFisio(aluno.id, fisioterapeutaId)
    if (resultado?.erro) {
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 p-4 pt-20">
      <div className="w-full max-w-md rounded-lg bg-white dark:bg-gray-900 shadow-xl">
        <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 px-4 py-3">
          <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Alocar aluno em {fisioterapeutaNome}</h2>
