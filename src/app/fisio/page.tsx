    ...
    <AppShell>
      <div className="flex w-full flex-1 flex-col lg:flex-row">
        ...
        <aside className="w-full shrink-0 border-t border-gray-200 bg-white p-5 lg:w-80 lg:border-l lg:border-t-0">
          <h2 className="mb-4 font-medium text-gray-900">Novo fisioterapeuta</h2>
          <FisioterapeutaForm />

          {todos.length > 0 && (
            <div className="mt-8">
              <h2 className="mb-2 font-medium text-gray-900">Cadastrados</h2>
              <div className="space-y-2">
                {todos.map((f) => (
                  <div key={f.id} className="flex items-center justify-between gap-2 rounded-md border border-gray-200 p-2 text-sm">
                    <span className={f.ativo ? 'text-gray-900' : 'text-gray-400'}>
                      {f.nome}
                      {!f.ativo && ' (inativo)'}
                    </span>
